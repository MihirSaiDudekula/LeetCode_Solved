...
public class Solution {
    ...
    public static int findPages(ArrayList<Integer> arr, int n, int m) {
        ...
        int lowestMaxPages = Collections.max(arr);
        int highestMaxPages = arr.stream().mapToInt(Integer::intValue).sum();

        int start = lowestMaxPages;
        int end = highestMaxPages;
        int result = -1;

        while (start <= end) {
            int mid = (start + end) / 2;
            int studentsCount = countStudents(arr, n, mid);

            if (studentsCount <= m) {
                result = mid; // mid could potentially be a valid answer
                end = mid - 1; // try for a smaller max limit
            } else {
                start = mid + 1; // need more max limit
            }
        }
    ...
    public static int countStudents(ArrayList<Integer> arrofPages, int n, int pageLimit) {
        int noOfStudents = 1;
        long pagesForThisStudent = 0;

        for (int i = 0; i < n; i++) {
            if (pagesForThisStudent + arrofPages.get(i) <= pageLimit) {
                pagesForThisStudent += arrofPages.get(i);
            } else {
                noOfStudents++;
                pagesForThisStudent = arrofPages.get(i);
            }
